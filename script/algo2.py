...
#checkpoint_C = torch.load("/data1/ymh/lrcl/resnet56_real5.pth")

#####################################################################################################################
#C = network.resnet56(num_classes=10).to(device)
C = Classifier().to(device)
C.load_state_dict(checkpoint_C['model_state_dict'])
A = Actor(3*32*32, 10, 84).to(device)
M = Filter().to(device)
M_list = list(M.modules())
Q = Critic(3*32*32, 10, 84).to(device)

A_t = Actor(3*32*32, 10, 84).to(device)
Q_t = Critic(3*32*32, 10, 84).to(device)
A_t.load_state_dict(A.state_dict())
Q_t.load_state_dict(Q.state_dict())

optim_C = optim.Adam(C.parameters(), lr=0.01)
optim_A = optim.Adam(A.parameters(), lr=0.0001)
optim_M = optim.Adam(M.parameters(), lr=0.01)
...
for i, (x,y) in enumerate(train_loader):
    ...
    for j in range(Budget):
        if (C(x).argmax(1, keepdim=True) == y and C(x).max(1, keepdim=True)[0] > Confidence) or (j+1) == Budget:
            ...
            if buffer.n_entries < exploration:
                action = torch.from_numpy(np.random.uniform(-1,1,84))
                action = action.float().to(device)
            else:
                delta = init_delta * (delta_decay ** (buffer.n_entries + 1 - exploration))
                noise = torch.from_numpy(np.random.normal(0, delta, 84))
                noise = noise.float().to(device)
                action = A([x,C(x)]).view(-1) + noise
                
            M_list[1].weight.data = action[:81].view(3,3,3,3)
            M_list[1].bias.data = action[81:]
            ...
            optim_M.step()
            weight = M_list[1].weight.data.view(-1)
            bias = M_list[1].bias.data.view(-1)
            action_1 = torch.cat((weight, bias))
            ...
            with torch.no_grad():
                ...
                for (x,y) in test_loader:
                    ...
                    for k in range(Budget):
                        if C(x).max(1, keepdim=True)[0] > Confidence or (k+1) == Budget:
